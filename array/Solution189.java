...
* 时间复杂度分析：需要循环k次，每次循环需要遍历数组所有元素，所以时间复杂度O(n) = 数组元素个数n*k；
* 空间复杂度分析：每次循环需要一个tmp变量，O(n) = n;
* 该方法运行超时；
* 代码如下：
...
* 时间复杂度分析:遍历一次数组长度，O(n) = 数组长度n；
* 空间复杂度分析：新建了一个数组，O(n) = 1；
* */
public class Solution189 {
    public void rotate(int[] nums, int k) {
        ...
        if (time >= 0) System.arraycopy(nums, len - time, numsNew, 0, time);
        if (len - time >= 0) System.arraycopy(nums, 0, numsNew, time, len - time);
